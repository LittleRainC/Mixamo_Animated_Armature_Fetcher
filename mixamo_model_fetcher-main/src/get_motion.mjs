...
async function fetchWithRetry(url, options={}, retries=5){
	for(let i=0; i<retries; ++i){
		try{
			...
			console.log(`\n[WARNING] fetch failed (${err.cause?.code || err.message}), retry ${i+1}/${retries}...`);
		...
		try{
			...
			while(true){
				...
				if(Date.now()-lastLog>3000){
					...
					const pct=total?` (${Math.min(100,(100*received/total)).toFixed(0)}%)`:'';
					process.stdout.write(`\r[STATUS] downloading ${mb}MB${pct}...`);
					lastLog=Date.now();
				}
			}
			...
			console.log('[SUCCESS] Download completed:', destPath, `(${(received/1024/1024).toFixed(1)}MB)`);
			return;
		}catch(err){
			fs.unlink(destPath, ()=>{});
			const msg=err.name==='AbortError'?'timeout':(err.code||err.message);
			if(attempt===retries-1) throw err;
			console.log(`\n[WARNING] download failed (${msg}), retry ${attempt+1}/${retries}...`);
...
export async function download_all_motions(options, characterId="f7b85d05-5f1c-47d8-9770-9a1a054bd6f6"){
    ...
    for(let i=startIdx;i<endIdx;++i){
        ...
        try{
            await download_motion(m, characterId, {dir:options.out});
        } catch(err){
            console.log(`[WARNING] motion ${i} failed (${err.code || err.message}), skipping...`);
        }
    }
}
...
async function download_motion(motion, characterId, {format='fbx7_2019', dir='./tmp', fps="30", reducekf="0", skin="false"}={}){
    ...
    if(download_url){
        ...
        const download_res=await step_download(download_url, path.join(dir, `${safeName}.${suffix}`));
        return true;
    }
    return false;
}
...
async function getGmsHash(motion, characterId){
    const res=await fetchWithRetry(`https://www.mixamo.com/api/v1/products/${motion.id}?similar=0&character_id=${characterId}`,{
        "headers": {
...
async function step_export(motion, characterId, {format="fbx7_2019", fps="30", reducekf="0", skin="false"}={}){
	...
	do{
		const res = await fetchWithRetry("https://www.mixamo.com/api/v1/animations/export", {
			"headers": {
			"authorization":process.env.AUTHKEY,
...
async function step_monitor(characterId){
	...
	let count=0;
	do{
		const res=await fetchWithRetry(`https://www.mixamo.com/api/v1/characters/${characterId}/monitor`, {
			"headers": {
				"authorization":process.env.AUTHKEY,
				"content-type": "application/json; charset=UTF-8",
				"x-api-key": "mixamo2",
...
async function step_download(url, destPath){
